...
// Data
const slots = ref<NodeListOf<Element>>([] as unknown as NodeListOf<Element>);
const room = ref<RoomType>({} as RoomType);
const loadingMoreRooms = ref<boolean>(false);
...
const castArray = <T>(val: T[] | string): T[] => {
	return !val ? [] : Array.isArray(val) ? val : JSON.parse(val as string);
};

const castObject = <T>(val: T | string): T => {
	return !val ? {} as T : typeof val === 'object' ? val : JSON.parse(val as string);
};
...
const roomsCasted = computed(() => castArray<RoomType>(props.rooms));
const messagesCasted = computed(() => castArray<MessageType>(props.messages));
const roomActionsCasted = computed(() => castArray<ActionType>(props.roomActions));
const menuActionsCasted = computed(() => castArray<ActionType>(props.menuActions));
const messageActionsCasted = computed(() => castArray<ActionType>(props.messageActions));
const messageSelectionActionsCasted = computed(() => castArray<ActionType>(props.messageSelectionActions));
const templatesTextCasted = computed(() => castArray<string>(props.templatesText));
const stylesCasted = computed(() => castObject<Record<string, any>>(props.styles));
const textMessagesCasted = computed(() => castObject<Record<string, any>>(props.textMessages));
const autoScrollCasted = computed(() => castObject<AutoScrollConfig>(props.autoScroll));
const textFormattingCasted = computed(() => castObject<TextFormattingConfig>(props.textFormatting));
const linkOptionsCasted = computed(() => castObject<LinkOptionsConfig>(props.linkOptions));
const usernameOptionsCasted = computed(() => castObject<UsernameOptionsConfig>(props.usernameOptions));
